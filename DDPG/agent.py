...
class Actor(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.tanh(self.linear3(x))               # 使输出范围限定在[-1, 1]
        return x
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim, init_w=3e-3):
        super(Critic, self).__init__()
        
        self.linear1 = nn.Linear(state_dim + action_dim, hidden_dim)   # 输入维度为 state_dim + action_dim
        self.linear2 = nn.Linear(hidden_dim, hidden_dim)
...
class DDPG:
    ...
    def update(self):
        ...
        value_loss.backward()
        self.critic_optimizer.step()              # 更新模型
        # 软更新
        for target_param, param in zip(self.target_critic.parameters(), self.critic.parameters()):
