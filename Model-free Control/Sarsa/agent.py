...
class Sarsa(object):
    ...
    def choose_action(self, state):
        ...
        action = np.random.choice(np.arange(len(action_probs)), p=action_probs)  # 在0-3中按照概率选择一个数
        return action
            
    def update(self, state, action, reward, next_state, next_action,done):      # Sarsa的核心算法
