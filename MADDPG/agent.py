...
class MADDPG:
    ...
    def choose_action(self, ob, env_name, env, agent_name, side_index, sfs, mode):
        ...
        if mode == 'train':
            if np.random.uniform() > self.epsilon:
                ...
                if env_name == 'simple_tag_v2' or env_name == 'simple_world_comm_v2':
                    if agent_name not in side_index:
                        # 从头训练切换
                        if sfs:
    ...
    def update(self, replay_buffer, agent_n):
        ...
        with torch.no_grad():  # target_Q 无梯度
            ...
            batch_a_next_n = [agent.target_actor(batch_obs_next) for agent, batch_obs_next in zip(agent_n, batch_obs_next_n)]
            Q_next = self.target_critic(batch_obs_next_n, batch_a_next_n)
    ...
    def save(self, side_name, path, ep_count, max_train_ep, max_ave_rew, agent_id):
        model_actor = "Actor_agent{}.pt".format(agent_id)
        model_critic = "Critic_agent{}.pt".format(agent_id)

        if side_name == 'red':
            model_actor = "red/Actor_agent{}.pt".format(agent_id)
            model_critic = "red/Critic_agent{}.pt".format(agent_id)
        if side_name == 'green':
            model_actor = "green/Actor_agent{}.pt".format(agent_id)
            model_critic = "green/Critic_agent{}.pt".format(agent_id)
        ...
        if side_name == 'red':
            write_txt_file(path + '/model/' + '/red/' + 'log.txt', record)      # 记录保存模型时的回合数
        elif side_name == 'green':
            write_txt_file(path + '/model/' + '/green/' + 'log.txt', record)      # 记录保存模型时的回合数
        else:
            write_txt_file(path + '/model/' + 'log.txt', record)                 # 记录保存模型时的回合数

    def load(self, side_name, path, agent_id):
        model_actor = "Actor_agent{}.pt".format(agent_id)
        model_critic = "Critic_agent{}.pt".format(agent_id)
