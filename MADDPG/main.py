...
from pettingzoo.mpe import simple_reference_v2, simple_spread_v2, simple_adversary_v2, simple_tag_v2, simple_world_comm_v2
...
env_name = ['simple_reference_v2', 'simple_spread_v2', 'simple_adversary_v2', 'simple_tag_v2', 'simple_world_comm_v2'][4]         # 环境名称
algo_name = 'MADDPG'                  # 算法名称
side_name = None                      # 设置推演方
start_from_scratch = sfs = False      # 是否从头博弈

if env_name == 'simple_tag_v2' or env_name == 'simple_world_comm_v2':         # 当环境为博弈环境时
    agent_red_num = 4                 # 设置红方智能体个数
    agent_green_num = 4               # 设置绿方智能体个数
    side_name = 'green'                 # 设置推演方
...
def train():
    ...
    # env = simple_tag_v2.parallel_env(num_good=agent_green_num, num_adversaries=agent_red_num, num_obstacles=2, max_cycles=25, continuous_actions=True)
    env = simple_world_comm_v2.parallel_env(num_good=agent_green_num, num_adversaries=agent_red_num, num_obstacles=2,
                          num_food=2, max_cycles=50, num_forests=3, continuous_actions=True)
    env = mpe_wrapper_for_pettingzoo(env, continuous_actions=True)

    n = env.num_agents                                     # 环境中智能体的数量
    agents = env.agents                                    # 环境中智能体列表
    if side_name == 'red':                                 # 推演方智能体的数量
        side_n = n - agent_green_num
        side_list = agents[:agent_red_num]
        side_index = [i for i in range(agent_red_num)]
    elif side_name == 'green':
        side_n = n - agent_red_num
        side_list = agents[-agent_green_num:]
        side_index = [i for i in range(agent_red_num, agent_red_num + agent_green_num)]
    else:
        side_n = n
        side_list = agents
        side_index = [i for i in range(n)]
    if agent_red_num:
        red_side_index = [i for i in range(agent_red_num)]
        green_side_index = [i for i in range(agent_red_num, agent_red_num + agent_green_num)]
    ...
    print('Information'+'\n' + \
          'name: '+ env_name+'\n' + 'agent_num: ' + str(n) + '\n' + \
          'side_num: ' + str(side_n) + '\n' + 'side_list: ' + str(side_list) + '\n' + \
          'ob_dim: '+ str(ob_dim_n) +'\n' + 'action_dim: '+ str(action_dim_n) +'\n' + 'state_dim: ' + str(state_dim) +'\n' + \
          'ob_type: '+ str(ob_type) +'\n' + 'action_type: '+ str(action_type) + '\n' + \
          # 'ob_high: '+ str(ob_high) +'\n' + 'ob_low: '+ str(ob_low) + '\n' + \
          'action_high: '+ str(action_high) + '\n' + 'action_low: ' + str(action_low))

    cfg = MADDPGConfig()                                   # 配置模型参数

    # 创建智能体
    agent_n = [MADDPG(ob_dim_n, action_dim_n, state_dim, cfg, agent_id) for agent_id in range(n)]

    total_step = 0                                       # 记录总步数
    epoch = []                                           # 记录轮数，方便画图
    rewards = []                                         # 记录所有回合的奖励
    ave_rewards = []                                     # 记录平均奖励值
    agent_rewards = [[] for _ in range(side_n)]               # 记录每个智能体的奖励值
    ...
    if start_ep == 0:
        if side_name == 'red':
            make_dir(outputs_path + '/img/', outputs_path + '/model/' + '/red/')  # 创建文件夹
        elif side_name == 'green':
            make_dir(outputs_path + '/img/', outputs_path + '/model/' + '/green/')  # 创建文件夹
        else:
        ...
        if sfs:
            ...
            if side_name == 'red':
                for agent_id in green_side_index:                                 # 加载模型
                    agent_n[agent_id].load(side_name='green', path=outputs_path, agent_id=agent_id + 1)

            if side_name == 'green':
                for agent_id in red_side_index:                                   # 加载模型
                    agent_n[agent_id].load(side_name='red', path=outputs_path, agent_id=agent_id + 1)
        ...
        for agent_id in side_index:                                   # 加载模型
            agent_n[agent_id].load(side_name, outputs_path, agent_id + 1)
    ...
    for ep in range(start_ep, cfg.train_eps):
        ep_reward = 0
        agent_reward = [0 for _ in range(side_n)]                        # 每回合每个智能体的奖励值
        ...
        for step in range(cfg.max_step):
            ...
            action_n = [agent.choose_action(ob, env_name, env, agent_name, side_index, sfs, mode='train') for agent, ob, agent_name in zip(agent_n, ob_n, range(n))]

            next_ob_n, reward_n, done_n, _ = env.step(action_n)

            for i, reward in enumerate(np.array(reward_n)[side_index]):
                agent_reward[i] += reward

            ep_reward += sum(np.array(reward_n)[side_index])

            for agent_id in range(side_n):
            ...
            if total_step % 200 == 0:                      # 每200步更新一次智能体
                for agent_id in side_index:                # 每个智能体单独更新
        ...
        for i in range(side_n):                                                   # 记录每个智能体每回合奖励值
        ...
        if (ep + 1) >= (start_ep + cfg.ep_r):                                             # 后几轮平均奖励值较大时，保存模型
            if np.mean(rewards[-int(cfg.ep_r):]) > max_ave_rew:
                ...
                for agent_id in side_index:                                   # 保存模型
                    agent_n[agent_id].save(side_name, outputs_path, ep + 1, cfg.train_eps, max_ave_rew, agent_id + 1)
...
def test():
    ...
    # 加载模型
    # 加载推演方模型
    for agent_id in side_index:
        agent_n[agent_id].load(side_name, outputs_path, agent_id + 1)

    if not sfs:
        # 加载对手模型
        if side_name == 'green':
            for agent_id in red_side_index:
                agent_n[agent_id].load(side_name='red', path=outputs_path, agent_id=agent_id + 1)

        if side_name == 'red':
            for agent_id in green_side_index:
                agent_n[agent_id].load(side_name='green', path=outputs_path, agent_id=agent_id + 1)
    ...
    for ep in range(cfg.test_eps):
        ...
        for step in range(cfg.max_step):
            action_n = [agent.choose_action(ob, env_name, env, agent_name, side_index, sfs, mode='test') for agent, ob, agent_name in zip(agent_n, ob_n, range(n))]
